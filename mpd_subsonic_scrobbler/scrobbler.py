...
def execute_scrobbling(
        context: Context,
        subsonic_server_config: SubsonicServerConfig,
        song: Song,
        index: int) -> dict:
...
def handle_playback(context: Context, index: int):
    ...
    subsonic_track_id_result: SubsonicTrackId = (subsonic_util.get_subsonic_track_id(
        context=context,
        index=index,
        scrobbler_config_list=context.get_config().get_server_list())
        if song_file
        else None)
    ...
    while True:
        ...
        for mpd_index in range(len(mpd_list)):
            ...
            try:
                ...
                # status will be saved in context, so we don't really need the returned dict
                mpd_util.get_mpd_status(context, mpd_index=mpd_index)

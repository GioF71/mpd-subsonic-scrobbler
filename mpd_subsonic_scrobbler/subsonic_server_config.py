...
class SubsonicServerConfig(Configuration):
    ...
    def __init__(self, index: int = 0):
        ...
        if self.__subsonic_file:
            ...
            if ConfigKey.SUBSONIC_CREDENTIALS.get_key() in self.__dict:
                ...
                cred_dict: dict[str, str] = dotenv.dotenv_values(
                    dotenv_path=self.__dict[ConfigKey.SUBSONIC_CREDENTIALS.get_key()])
    ...
    def get_friendly_name(self) -> str:
        if ConfigKey.SUBSONIC_FRIENDLY_NAME.get_key() in self.__dict:
            return self.__dict[ConfigKey.SUBSONIC_FRIENDLY_NAME.get_key()]
